...
class GitHubCollector:
    ...
    async def collect(self, request: CollectRequest) -> CollectResult:
        ...
        page = int(request.checkpoint.cursor) if request.checkpoint and request.checkpoint.cursor else 1
        ...
        try:
            payload = await bounded_get_json(
                ...
                budget=budget,
                params={"q": query, "sort": "updated", "order": "desc", "per_page": min(100, request.max_signals), "page": page},
            ...
            return CollectResult(
                ...
                request_count=budget.used,
                warnings=(SourceWarning(code="GITHUB_UNAVAILABLE", message=str(exc), retryable=True),),
            )
    ...
    def _is_bot(raw: dict[str, object]) -> bool:
        ...
        return isinstance(user, dict) and (
            str(user.get("type", "")).lower() == "bot" or str(user.get("login", "")).lower().endswith("[bot]")
        )
    ...
    def _normalize_issue(cls, raw: object) -> CollectedItem | None:
        ...
        lowered = body.lower()
        if not title or GENERIC_BUG.match(title.strip()) or sum(marker in lowered for marker in TEMPLATE_MARKERS) >= 2:
            return None
        user = raw.get("user")
        ...
        return CollectedItem(
            ...
            source_edited_at=utc_from_timestamp(raw["updated_at"]) if raw.get("updated_at") else None,
            engagement=Engagement(comments=max(0, int(raw.get("comments") or 0)), reactions=0),
            metadata={"repository_url": raw.get("repository_url"), "number": raw.get("number")},
        )
