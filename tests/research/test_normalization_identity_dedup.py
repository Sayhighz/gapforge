...
from gapforge.analysis.deduplication import dedup_key, exact_duplicate_reason, minhash_signature, minhash_similarity
from gapforge.analysis.identity import pseudonymize_author
from gapforge.analysis.normalization import append_revision, normalize_text, normalize_url
...
def item(identifier: str, url: str, body: str) -> CollectedItem:
    return CollectedItem(
        source=Source.GITHUB, external_id=identifier, canonical_url=url,
        body=body, source_created_at=NOW,
    )
...
def test_minhash_near_duplicate_is_stable_and_bounded() -> None:
    ...
    right = minhash_signature("manual invoice export takes several hours every friday for accountants")
...
def test_edits_and_tombstones_append_history_without_duplicate_revisions() -> None:
    ...
    unchanged = append_revision(history, raw_signal_id="raw-1", title=" pain ", body="OLD", observed_at=NOW)
    edited = append_revision(unchanged, raw_signal_id="raw-1", title="Pain", body="new", observed_at=NOW)
    deleted = append_revision(edited, raw_signal_id="raw-1", title=None, body=None, observed_at=NOW, deleted=True)
