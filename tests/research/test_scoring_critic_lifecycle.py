...
from gapforge.analysis.critic import (
    build_critic_input,
    research_more_intents,
...
def score(value: float = 100):
    inputs = ScoringInputs(
        severity=value,
        frequency=value,
        independent_diversity=value,
        behavioral_workaround=value,
        wtp_or_spend=value,
        recency_trend=value,
        gap_strength=value,
        competitor_dissatisfaction=value,
        reachability=value,
        technical_feasibility=value,
        small_team_feasibility=value,
        inverse_switching_friction=value,
        why_now=value,
    )
...
def test_geometric_mean_penalties_explanation_and_append_only_history() -> None:
    asymmetric = ScoringInputs(
        ...
        small_team_feasibility=25,
        inverse_switching_friction=25,
        why_now=25,
        penalties=(("concentration", 5),),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"card": card().model_copy(update={"known_author_ids": ("a",)})},
            "known_authors",
        ),
        (
            {"card": card().model_copy(update={"thread_ids": ("t",)})},
            "independent_threads",
        ),
        (
            {"card": card().model_copy(update={"user_sources": (Source.GITHUB,)})},
            "user_sources",
        ),
        (
            {"card": card().model_copy(update={"behavioral_workarounds": 0})},
            "behavioral_workaround",
        ),
        (
            {"card": card().model_copy(update={"paid_or_wtp_signals": 0})},
            "wtp_or_spend",
        ),
        (
            {"competitor_research": CompetitorResearchStatus.INCOMPLETE},
            "competitor_research",
        ),
        ...
        (
            {"card": card().model_copy(update={"confidence": 0.5})},
...
def test_each_validate_hard_gate_fails_independently(
...
def hypothesis() -> ProblemHypothesis:
    return ProblemHypothesis(
        id="p-1",
        canonical_problem_id="cp-1",
        icp="accountants",
        job_to_be_done="reconcile invoices",
        trigger="month end",
        current_behavior="manual CSV exports",
        pain="takes a day",
        workflow_failure="systems do not reconcile",
        falsification_test="Fails if fewer than five users confirm",
...
def gap() -> GapHypothesis:
    return GapHypothesis(
        id="g-1",
        canonical_problem_id="cp-1",
        gap_type=GapType.WORKFLOW,
        statement="No end-to-end reconciliation",
        user_evidence_ids=("e-1",),
        competitor_evidence_ids=("e-2",),
    )
...
def test_critic_input_is_blind_schema_and_citations_are_allowlisted() -> None:
    ...
    blind = build_critic_input(
        ...
        evidence_card=card(),
        gap_hypothesis=gap(),
        competitor_claims=(claim,),
        permitted_claim_ids=("c-1",),
    )
...
def test_hypotheses_are_falsifiable_allowlisted_and_cover_nonsoftware_alternatives() -> None:
    ...
    validate_gap_hypothesis(
        ...
        permitted_competitor_evidence=frozenset({"e-2"}),
    )
...
def test_trend_requires_five_signals_three_authors_two_threads_and_volume_normalization() -> None:
    ...
    assert (
        classify_trend(
            viral, as_of=NOW, current_examined_volume=100, previous_examined_volume=100
        ).label
        is TrendLabel.INSUFFICIENT_DATA
    )
    diverse = tuple(
        TrendObservation(f"s-{i}", f"a-{i % 3}", f"t-{i % 2}", NOW - timedelta(days=1))
        for i in range(6)
    )
    result = classify_trend(
        diverse, as_of=NOW, current_examined_volume=20, previous_examined_volume=100
    )
    ...
    duplicate_result = classify_trend(
        copies, as_of=NOW, current_examined_volume=20, previous_examined_volume=100
    )
    assert duplicate_result.current_signals == 1
...
def test_validation_rejects_cross_opportunity_artifacts_and_low_snapshot_confidence() -> None:
    with pytest.raises(ValueError, match="different opportunities"):
        validation_decision(
            ...
            competitor_research=CompetitorResearchStatus.COMPLETE,
            gap_evidence_present=True,
            critic=critic(opportunity_id="other"),
        )
    low = score().model_copy(update={"evidence_confidence": 0.5})
...
def test_reopen_cooldown_wtp_exception_and_lifecycle_guard() -> None:
    ...
    assert not can_reopen_rejected(
        rejected_at=rejected, now=NOW, evidence=ReopenEvidence(new_independent_users=3)
    )
    assert can_reopen_rejected(
        rejected_at=rejected, now=NOW, evidence=ReopenEvidence(first_wtp_or_spend=True)
    )
    assert (
        transition_lifecycle(
            LifecycleState.REJECTED, LifecycleState.RESEARCH_MORE, reopen_allowed=True
        )
