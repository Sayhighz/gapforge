...
from gapforge.search.brave import (
    ApprovedUrlRegistry,
    BraveSearchProvider,
...
async def test_fetch_blocks_private_loopback_link_local_and_metadata(
    address: str,
) -> None:
    calls = 0
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        result = await StaticFetcher(client, resolver=resolver).fetch(
            "https://example.com/page", approved_registry()
        )
...
async def test_fetch_pins_validated_ip_and_extracts_visible_text() -> None:
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        result = await StaticFetcher(client, resolver=public_resolver).fetch(
            "https://example.com/page", approved_registry()
        )
...
async def test_oversized_unsupported_and_agent_created_urls_fail_closed() -> None:
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        ...
        unsupported = await fetcher.fetch(
            "https://example.com/pdf", approved_registry("https://example.com/pdf")
        )
