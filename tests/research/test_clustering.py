...
def observation(
    index: int, *, author: str | None, thread: str, duplicate: str, spend: bool = False
) -> Observation:
...
def test_repeat_author_duplicate_and_viral_thread_cannot_fake_independence() -> None:
    repeats = tuple(
        observation(index, author="same", thread="viral", duplicate=f"d-{index}")
    ...
    duplicates = tuple(
        observation(index, author=f"a-{index}", thread=f"t-{index}", duplicate="same")
...
def test_cluster_and_opportunity_thresholds() -> None:
    two = (
        observation(1, author="a", thread="t1", duplicate="d1"),
        observation(2, author="b", thread="t1", duplicate="d2"),
    )
    ...
    assert (
        opportunity_entry_verdict(
            (observation(4, author="a", thread="t1", duplicate="d4", spend=True),)
        )
        is Verdict.RESEARCH_MORE
    )
...
def test_dormancy_and_merge_reversal_are_deterministic() -> None:
    ...
    candidate = MergeCandidate(
        id="m-1",
        left_problem_id="p-1",
        right_problem_id="p-2",
        similarity=0.8,
        rationale="lexical overlap",
    )
    accept = MergeDecision(
        candidate_id="m-1",
        action="ACCEPT",
        actor="operator",
    ...
    reverse = MergeDecision(
        candidate_id="m-1",
        action="REVERSE",
        actor="operator",
        reason="different ICP",
        decided_at=NOW,
    )
