...
from gapforge.domain.contracts import AtomicClaim, Citation, ClaimKind, EpistemicStatus, PainSignal, Source
...
def record() -> EvidenceRecord:
    return EvidenceRecord(
        "e-1", Source.STATIC_WEB, "https://acme.example/pricing",
        "The Pro plan costs $20 per month and includes exports.", NOW,
    )


def test_pain_extraction_requires_source_id_excerpt_and_grounded_context() -> None:
    source = EvidenceRecord(
        "raw-1:r1", Source.GITHUB, "https://github.com/acme/app/issues/1",
        "As an accountant I export CSV files manually every Friday.", NOW,
    )
    valid = PainSignal(
        id="p-1", raw_signal_revision_id="raw-1:r1", pain="Reconciliation is manual",
        user_context="accountant", workaround="export CSV files manually", severity=0.8,
        frequency=0.7, confidence=0.9, excerpt="I export CSV files manually every Friday",
    )
...
def test_evidence_card_deduplicates_before_independence_metrics() -> None:
    observations = tuple(
        EvidenceObservation(
            evidence_id=f"e-{index}", duplicate_group="viral-copy", author_id=f"a-{index}",
            thread_id=f"t-{index}", source=Source.GITHUB, observed_at=NOW, severity=1,
            behavioral_workaround=True, paid_or_wtp=True,
        )
...
def test_evidence_card_captures_time_source_and_contradiction_diversity() -> None:
    observations = (
        EvidenceObservation("e-1", "d-1", "a-1", "t-1", Source.GITHUB, NOW, 0.8, supporting_claim_ids=("c-1",)),
        EvidenceObservation("e-2", "d-2", "a-2", "t-2", Source.REDDIT, NOW - timedelta(days=20), 0.4, contradicting_claim_ids=("c-2",)),
    )
    card = build_evidence_card("card-1", "o-1", observations)
...
def test_claim_validation_rejects_invented_id_url_excerpt_time_and_contradiction() -> None:
    citation = Citation(evidence_id="e-1", source_url="https://attacker.example/", excerpt="$99 forever", observed_at=NOW - timedelta(days=1))
    claim = AtomicClaim(
        id="c-1", text="Acme costs $20", kind=ClaimKind.PRICE, status=EpistemicStatus.SUPPORTED,
        evidence_ids=("e-1", "invented"), citations=(citation,), contradicts_claim_ids=("missing",),
    )
...
def test_valid_captured_price_claim_and_single_repair_marker() -> None:
    source = record()
    claim = AtomicClaim(
        id="c-1", text="Acme Pro costs $20 monthly", kind=ClaimKind.PRICE,
        status=EpistemicStatus.SUPPORTED, evidence_ids=("e-1",),
        citations=(Citation(evidence_id="e-1", source_url=source.url, excerpt="Pro plan costs $20 per month", observed_at=NOW),),
    )
